...
PATTERN_REFACTOR_ADVICE_DEFAULTS: dict[str, str] = {
    ...
    "Adapter": "Wrap incompatible interfaces with an adapter class; convert the expected methods and types.",
    ...
    "Bridge": "Split abstraction from implementation; define an interface and inject a concrete implementor.",
    ...
    "Delegation Pattern": "Forward work to a helper object instead of inheritance; pass through required APIs.",
    "Dependency Injection": "Inject dependencies via constructor or setters; avoid globals and service locators.",
    ...
    "Hierarchical State Machine (HSM)": "Model nested states with explicit enter/exit and parent/child transitions; consider State objects for each level.",
    ...
    "Singleton": "Control instance via __new__/class attr; provide get_instance; keep it stateless where possible.",
...
ARCHITECTURE_REFACTOR_ADVICE_DEFAULTS: dict[str, str] = {
    "Clean Architecture": "Organize concentric layers; entities and use-cases core; frameworks at the outer rings.",
    ...
    "Hexagonal Architecture": "Introduce ports (interfaces) and adapters; push IO to edges; drive the domain via use cases.",
    "Layered Architecture": "Separate presentation, application, domain, and infrastructure; depend inward; define clear boundaries.",
    "Message Bus": "Send commands/events over a bus; handlers are decoupled; support retries and dead-letter queues.",
    ...
    "Service Layer": "Expose application services orchestrating use-cases; keep domain logic in entities/value objects.",
    "Three-Tier Architecture": "Separate presentation, logic, and data tiers; enforce strict dependencies and contracts.",
