...
def test_detects_front_controller():
    code = (Path(__file__).parent / "samples" / "front_controller_good.py").read_text()
    res = analyze_code_for_patterns(code, registry)
    assert any(r.get("name") == "Front Controller" for r in res)
